 ...
 * 临时调试用：页面加载即显示一个绿色信息框（viewport/媒体查询/html 滤镜
 * 等），打开 .h8S2Va_menu / .JObwrW_panel 时追加弹层与触发器实测矩形、
 * computed 定位、fixed-CB 祖先检测结果。数字供人工回报，修好后删除。 */
(function () {
  "use strict";
  if (typeof window === "undefined") return;
  var TARGETS = [".h8S2Va_menu", ".JObwrW_panel"];
  var box = null;
  var last = "";
  function rect(r) { return "L" + Math.round(r.left) + " T" + Math.round(r.top) + " W" + Math.round(r.width) + " H" + Math.round(r.height) + " R" + Math.round(r.right) + " B" + Math.round(r.bottom); }
  function hasCBAncestor(el) {
    var n = el, out = [];
    var props = ["transform", "perspective", "contain", "filter", "backdrop-filter", "will-change", "zoom"];
    while (n && n.nodeType === 1) {
      var cs = window.getComputedStyle(n);
      for (var i = 0; i < props.length; i++) {
        var v = cs.getPropertyValue(props[i]);
        if (v && v !== "none" && v !== "auto") { out.push(n.tagName.toLowerCase() + (n.className && typeof n.className === "string" ? "." + n.className.split(" ").slice(0, 2).join(".") : "") + "[" + props[i] + "=" + v + "]"); break; }
      }
      n = n.parentElement;
    }
    return out;
  ...
  function update() {
    var rows = [];
    var vw = window.innerWidth, vh = window.innerHeight;
    rows.push("PROBE v2  " + new Date().toLocaleTimeString());
    rows.push("viewport: " + vw + "x" + vh + "  media<=480: " + window.matchMedia("(max-width: 480px)").matches);
    var h = window.getComputedStyle(document.documentElement);
    rows.push("html: transform=" + h.transform + " filter=" + h.filter + " zoom=" + h.zoom);
    TARGETS.forEach(function (sel) {
      var el = document.querySelector(sel);
      if (!el) { rows.push(sel + ": (closed)"); return; }
      var cs = window.getComputedStyle(el);
      var t = el.parentElement;
      rows.push(sel + ": " + rect(el.getBoundingClientRect()));
      rows.push("  computed: pos=" + cs.position + " L=" + cs.left + " T=" + cs.top + " R=" + cs.right + " B=" + cs.bottom + " W=" + cs.width + " H=" + cs.height + " maxH=" + cs.maxHeight + " z=" + cs.zIndex);
      if (t) rows.push("  trigger: " + rect(t.getBoundingClientRect()));
      var cb = hasCBAncestor(el);
      rows.push("  fixedCB: " + (cb.length ? cb.join(" | ") : "none"));
      rows.push("  inlineL/T: " + el.style.left + " / " + el.style.top);
    });
    var text = rows.join("\n");
    if (text === last) return;
    last = text;
    if (!box) {
      ...
      box.style.cssText = "position:fixed;top:4px;left:4px;z-index:2147483647;background:rgba(0,0,0,.85);color:#0f0;font:10px/1.4 monospace;padding:5px 7px;border-radius:6px;max-width:calc(100vw - 8px);max-height:40vh;overflow:hidden;white-space:pre;pointer-events:none;";
